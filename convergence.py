...
if __name__ == "__main__":
    ...
    for _ in range(refinements):
        ...
        mu = 1
        tau = np.zeros(mesh.n_cells)
        for i in range(mesh.n_cells):
            tau[i] = inp.supg_tuning_value / max(abs(mu) / mesh.h[i], inp.sigma_t[mesh.mat_id[i]])

        A = assemble_transport_matrix(mu, mesh, inp, tau)
        b = assemble_source(mu, mesh, inp, tau)
        u = spsolve(A, b)
