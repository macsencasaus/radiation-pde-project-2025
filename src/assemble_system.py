...
def assemble_source(mu: float, mesh: Mesh, data: InputData, tau: np.ndarray) -> np.ndarray:
    ...
    for i in range(1, mesh.n_points - 1):
        left_side = data.source[mesh.mat_id[i - 1]] * mesh.h[i - 1] \
            + 2 * data.source[mesh.mat_id[i - 1]] * tau[i - 1] * mu 
        right_side = data.source[mesh.mat_id[i]] * mesh.h[i] \
            - 2 * data.source[mesh.mat_id[i]] * tau[i] * mu 
        bs[i] = (left_side + right_side) / 2

    bs[0] = mesh.h[0] * data.source[0] / 2 \
        + (0.5 * (np.abs(mu) + mu) * data.boundary_values[0]) \
        - data.source[0] * tau[0] * mu 
    bs[-1] = mesh.h[-1] * data.source[-1] / 2 \
        + (0.5 * (np.abs(mu) - mu) * data.boundary_values[1]) \
        + data.source[-1] * tau[-1] * mu 
...
def assemble_transport_matrix(mu: float, mesh: Mesh, data: InputData, tau: np.ndarray) -> csr_matrix:
    ...
    for i in range(mesh.n_points):
        if i == 0:
            ...
            matrix_data[dataIdx + 1] = mu / 2 + data.sigma_t[0] * mesh.h[1]/6 \
                - (mu * mu) * tau[1] / mesh.h[1] - data.sigma_t[0] * tau[1] * mu / 2
            dataIdx += 2
            pass
        elif i == mesh.n_points - 1:
            # lower diagonal (i == j + 1)
            matrix_data[dataIdx] = -mu / 2 + data.sigma_t[-1] * mesh.h[-2] / 6 \
                - (mu * mu) * tau[-2] / mesh.h[-2] + data.sigma_t[-1] * tau[-2] * mu / 2
