...
def compute_initial_guess(mesh: Mesh, data: InputData) -> np.ndarray:
    ...
    r = [data.source[mesh.mat_id[cell]]  for cell in mesh.cells]
    sig_a = [data.sigma_t[mesh.mat_id[cell]] - data.sigma_s[mesh.mat_id[cell]] for cell in mesh.cells]
...
def compute_delta(phi: np.ndarray, phi_half: np.ndarray, mesh: Mesh, data: InputData) -> np.ndarray:
    sig_s = [data.sigma_s[mesh.mat_id[cell]] + 1e-10 for cell in mesh.cells]
    sig_t = [data.sigma_t[mesh.mat_id[cell]] for cell in mesh.cells]
    sig_a = [sig_t[i] - sig_s[i] for i in range(len(sig_s))]
